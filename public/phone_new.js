...
let defaultAvatars = getDbItem("defaultAvatars", "avatars/default.0.webp"); 
...
let userAgentStr = getDbItem("UserAgentStr", "WebRTC"+ appversion +" (SIPJS - "+ sipjsversion +") "+ navUserAgent);   // Set this to whatever you want.
...
function InitUi(){
    ...
    // Custom Web hook
    console.log("sip new registration= "+JSON.stringify(phoneOptions));
...
function onTransportConnected(){
    ...
    /*** Create Log File */
    logEntry("Connected to Web Socket!");
...
function onTransportConnectError(error){
    ...
    /***Create log file */
    logEntry("WebSocket Connection Failed: ");
...
function onTransportDisconnected(){
    ...
    ///*** create log file */
    logEntry("Disconnected from Web Socket! ");
    $("#regStatus").html(lang.disconnected_from_web_socket);
...
function Unregister(skipUnsubscribe) {
    ...
    console.log("Unregister...");
     ///*** create log file */
    logEntry("Unregister Web Socket! ");
...
function ReceiveCall(session) 
{
    ...
    /**********Mini Dial ans and reject button*************** */
   /****mini_dialpad('incoming_call');*****/

    //alert('hello received');
    var UserMode=window.localStorage.getItem('UserMode');
    var WaitingDisposition=window.localStorage.getItem('WaitingDisposition');
    localStorage.setItem("isLineBusy",'RINGING');
    var isLineBusy=window.localStorage.getItem('isLineBusy');
    var callerID = session.remoteIdentity.displayName;
    var did = session.remoteIdentity.uri.user;
    //alert(UserMode);
    if(UserMode != 'Auto' && UserMode == 'Break' && did.length>5)
    {
        console.log("Incoming call only auto mode");
        //alert("Incoming call only auto mode");
        return false;
    }
    if(UserMode == 'Break' && did.length==5)
    {
        console.log("Incoming call not allow");
        //alert("Incoming call not allow");
        return false;
    }
    if(WaitingDisposition == 'true' && isLineBusy != 'READY')
    {
    ...
    console.log("incoming sip sipHeaders domain= ",sipHeaders);
    // If a P-Asserted-Identity is parsed, use that
    if(sipHeaders.hasOwnProperty("From")){
        var rawUri = sipHeaders["From"][0].raw;
        if(rawUri.includes("<sip:")) {
            var uriParts = rawUri.split("<sip:");
            var uriDetails=uriParts[1].split("@");
            var uriIP=uriDetails[1].split(">");
            console.log("incoming sip rawUri Domain IP1= ",uriIP[0]);
            console.log("incoming sip domain2= ",SipDomain);
            console.log("incoming sip astcontrolserver_url= "+astcontrolserver_url+" local= "+astcontrolserver_urllocal);
            //console.log("incoming sip domain3= ",uriParts[1].endsWith("@"+SipDomain));
            //if(uriParts[1].endsWith(">")) uriParts[1] = uriParts[1].substring(0, uriParts[1].length -1);
            if(uriIP[0]==astcontrolserver_urllocal || SipDomain==astcontrolserver_url){
                //var assertId = SIP.UserAgent.makeURI("sip:"+ uriParts[0]+"@"+uriIP[0]); // should be sip:123@domain.com
                //console.log("Found P-AssertedS = ","sip:"+ uriDetails[0]+"@"+uriIP[0]);
                //console.log("Found P-Asserted = ",assertId);
                //did = assertId.user;
                did=uriDetails[0];
                console.log("Found P-Asserted-Identity, will use that to identify user:", did);
                /** open dialog */
                var element2 = document.getElementById("my_offcanvas2");
                ...
                /** Enter log for Dilar */
                logEntry("Block Incoming call request");
                /** end log */
                return false;
            }
        }
    ...
    if(sipHeaders.hasOwnProperty("P-Asserted-Identity")){
        ...
        if(rawUri.includes("<sip:")) {
            var uriParts = rawUri.split("<sip:");
            //console.log("incoming sip rawUri domain= ",rawUri);
            if(uriParts[1].endsWith(">")) uriParts[1] = uriParts[1].substring(0, uriParts[1].length -1);
    ...
    console.log("New Incoming Call!", callerID +" <"+ did +">");
     /** Enter log for Dilar */
     logEntry("Incoming call request "+did);
     /** end log */
    const new_caller_data = callerID.split("-");
    if(new_caller_data[0]=='EC')
    {
        setLocalVariable('DisplayCallerID',new_caller_data[5]);
    }else
    {
        setLocalVariable('DisplayCallerID',new_caller_data[1]);
    }
    setLocalVariable('CALL_TYPE',new_caller_data[0]);
    setLocalVariable('RecordID',new_caller_data[3]);
    ...
    if(buddyObj == null) {
        ...
        if(buddyObj.type == "extension" && buddyObj.CallerIDName != callerID){
            UpdateBuddyCallerID(buddyObj, callerID, did);
    ...
    console.log("Incoming callerid= "+callerID);
    console.log("Incoming did= "+callerID);
    console.log("Incoming buddyobj= ",buddyObj);
    console.log("Webrtc_dialline buddyobj= ",newLineNumber);

    setLocalVariable('Webrtc_dialline',newLineNumber);
	...
	console.log("Autoanswered data= "+AutoAnswer_cred);
	//if(AutoAnswerEnabled || AutoAnswerPolicy == "enabled" || autoAnswerRequested){
    if(AutoAnswer_cred==1){
		
		console.log("AutoAnswerEnabled= "+AutoAnswerEnabled);
		console.log("AutoAnswerPolicy= "+AutoAnswerPolicy);
		console.log("autoAnswerRequested= "+autoAnswerRequested);
    ...
    if(EnableRingtone == true){
        if(CurrentCalls >= 1){
            ...
            // Play Ring Tone
            audioBlobs.Ringtone = { file : "Ringtone_1.mp3", url : hostingPrefix +"media/Ringtone_1.mp3" }
           console.log("Audio call ring:", audioBlobs.Ringtone.url);
            var ringer = new Audio(audioBlobs.Ringtone.url);
            ringer.preload = "auto";
            ringer.loop = true;
            ringer.oncanplaythrough = function(e) {
                if (typeof ringer.sinkId !== 'undefined' && getRingerOutputID() != "default") {
...
async function AnswerAudioCall(lineNumber) {
    // CloseWindow();
    if (getLocalVariable('reviewMode') == 1 || getLocalVariable('reviewMode') == "1") {
        console.log("Review Mode on");
        return false ;
    ...
    localStorage.setItem("WaitingDisposition",'true');
    setLocalVariable('Webrtc_dialline',lineNumber);
    var lineObj = FindLineByNumber(lineNumber);

    /**********Answer Audio calls*********** */
    $("#answer_call").css("display", "none"); 
    /***********End Answer audio calls************* */
    /**Start set dataid */
    await get_dataid(lineObj.DisplayNumber);
    /** end set dataid */
    /** Enter log for Dilar */
    logEntry("Answer incoming call"+lineObj.DisplayNumber);
    /** end log */
    const caller_data = lineObj.DisplayName.split("-");
    console.log("webrtc call type= ",caller_data);
    localStorage.setItem("isHold",'0');
	localStorage.setItem("MuteCall",'0');
    localStorage.setItem("NumberToDial", lineObj.DisplayNumber);
    crm_data_id=getLocalVariable("CRMDataID");
	$('#dial_crm_number').val(lineObj.DisplayNumber);
    $('#crm_phone_number').html(getLocalVariable('isDisplayPhone') === 'false' || getLocalVariable('isDisplayPhone') === false ? 'XXXXXXXXXX<img src="assets/images/call.svg" width="20" height="20" id="crm_redial" ondblclick=DialByLine("audio","","'+lineObj.DisplayNumber+'","","","RD","'+crm_data_id+'")>' : lineObj.DisplayNumber+'<img src="assets/images/call.svg" width="20" height="20" id="crm_redial" ondblclick=DialByLine("audio","","'+lineObj.DisplayNumber+'","","","RD","'+crm_data_id+'")>');
    if(caller_data[0]=='CC' || caller_data[0]=='TC' || caller_data[0]=='TQ')
    {
        var r_lid=getLocalVariable('LoginHourID');
        var r_mode_id=getLocalVariable('ModeDBId');
        var call_rid=caller_data[3];
        var to_campaign=getLocalVariable('CampaignName');
        var to_user='';
        if(caller_data[0]=='TQ')
        {
            //alert("call type "+caller_data[0]);
            to_user=getLocalVariable("USER_ID");
        }
        var new_json_data={r_lid:r_lid,r_mode_id:r_mode_id,call_rid:call_rid,to_campaign:to_campaign,to_user:to_user};
        //alert("json data "+JSON.stringify(new_json_data));
        try {
            let response = await fetch('/update_tcc_details', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(new_json_data)
            });
            ...
            if (data.status == 1) {
                //alertify.success("update_tcc_details");
            }
        } catch (error) {
    ...
    $('#crm_did_number').html(getLocalVariable('DisplayCallerID'));
    $('#crm_did_name').html(getLocalVariable('DisplayCallerID'));
    $('#crm_rid').html(caller_data[3]);
    $('#crm_rid').html(caller_data[3]);
    
    $('#top-crm-continer').css("display", "flex"); 
    $('#top-crm-continer2').css("display", "none");
    setLocalVariable('RecordID',caller_data[3]);
    setLocalVariable('CALL_TYPE',caller_data[0]);
    $('#crm_skill').html(caller_data[6]);
    if(caller_data[0]=='TQ')
    {
        $('#show_did_name').show();
        $('#webrtct_did_name').html(caller_data[5]);
    }
    ...
    lineObj.SipSession.accept(spdOptions).then(function(){
        onInviteAccepted(lineObj,false);
         /*** start outbound counter */
         queueAdd('queue_active');
         var CALL_TYPE=window.localStorage.getItem('CALL_TYPE');
         if(CALL_TYPE=='IB')
         {
            var ib_count=parseInt(window.localStorage.getItem('Call_In_Count'));
            var ib_counter=ib_count+1;
            localStorage.setItem("Call_In_Count", ib_counter);
         }
         else if(CALL_TYPE=='OB')
        {
            var ob_count=parseInt(window.localStorage.getItem('Call_Ob_Count'));
            var ob_counter=ob_count+1;
            localStorage.setItem("Call_Ob_Count", ob_counter);
        }
        else if(CALL_TYPE=='CC')
        {
            var cc_count=parseInt(window.localStorage.getItem('Conference_Count'));
            var cc_counter=cc_count+1;
            localStorage.setItem("Conference_Count", cc_counter);
        }
        else if(CALL_TYPE=='TC')
        {
            let tc_count = parseInt(window.localStorage.getItem('Transfer_Count'));
            let tc_counter = tc_count + 1;
            localStorage.setItem("Transfer_Count", tc_counter);
            /****Transfer Count data****/
            update_transfer_data(tc_counter);
        }
        else
        {
            var ib_count=parseInt(window.localStorage.getItem('Call_In_Count'));
    ...
    console.log("Incoming Call Crm Load"+caller_data[3]+"-"+lineObj.DisplayNumber+"-"+caller_data[1]);
    webrtc_crm(caller_data[3],lineObj.DisplayNumber,caller_data[0]);
    insert_disposition(caller_data[3],caller_data[0],lineObj.DisplayNumber,caller_data[1]);
    /** ADD queue data  by Alok**/
...
function RejectCall(lineNumber) {
    /** Enter log for Dilar */
    logEntry("Reject Call "+lineNumber);
...
function onInviteAccepted(lineObj, includeVideo, response){
    ...
    /** Enter log for Dilar */
    logEntry("Call Accepted");
    /** end log */
    // Call in progress
    console.log("Alok Call Progress...");
    /** close dialpad */
    var element = document.getElementById("my_offcanvas2");
    element.classList.remove("show");
    /**********Mini Dial ans and reject button*************** */
   /***mini_dialpad();****/ 
    ...
    if (websocketConnected == 1) 
    {
        ...
        console.log("Alok Call socket Progress...");
    }
    var session = lineObj.SipSession;
...
function onInviteProgress(lineObj, response){
    ...
    localStorage.setItem("isLineBusy",'DIALING');
...
function get_infostarttimer(totalSeconds)
{
...
function BargeCall(numDial) {
    if (websocketConnected == 0) {
        alertify.error("Control server connection failed.");
        return;
    }
    if (getLocalVariable('UserMode') == '' || getLocalVariable('CampaignName') == '') {
        alertify.error("The campaign name or user mode were chosen incorrectly.");
        return false;
    }

    /** Enter log for Dilar */
    logEntry("BargeCall "+numDial);
    /** end log */
    ShowContacts();
    newLineNumber=1;
    var  buddy='';
    ...
    var CallerID='GALAXY';
    var buddyObj={CallerIDName:'GALAXY'};
    if(buddyObj == null) {
        ...
        buddyObj = MakeBuddy(buddyType, true, false, false, (CallerID)? CallerID : "GALAXY", "GALAXY", null, false, null, AutoDeleteDefault, false);
    }
	...
	//console.log("webrtc sip_headers_value= "+ sip_headers_value);
	//spdOptions.extraHeaders.push('dcgalaxy' + ": "+sip_headers_value);
    ...
    var inviterOptions = {
        requestDelegate: { // OutgoingRequestDelegate
            ...
            onAccept:function(sip){
                ...
                /** Enter log for Dilar */
                logEntry("Barge Call Accepted= "+numDial);
                /** end log */
...
function AudioCall(lineObj, dialledNumber, extraHeaders, number_dial='', call_type='') {
    if (websocketConnected == 0) {
        alertify.error("Control server connection failed.");
        return;
    }
    ...
    var call_manual=parseInt(window.localStorage.getItem('Call_Mc_Count'));
    var count_manual=(call_manual+1);
    localStorage.setItem("Call_Mc_Count", count_manual);
    localStorage.setItem("WaitingDisposition",'true');
	/*SIp Calling*/
	datetime_sip=formatDate();
	var Campaign_name=window.localStorage.getItem('CampaignName');
	var callerid=window.localStorage.getItem('ManualCallerID');
    setLocalVariable('DisplayCallerID',callerid);
    var SIP_ID=window.localStorage.getItem('SIP_ID');
    var USER_ID=window.localStorage.getItem('USER_ID');
	var RecordID=datetime_sip+SIP_ID+"-SIP";
	...
	var Dynamic_Campaign_prefix=window.localStorage.getItem('Campaign_PreFix');
    var CRMDataID=1;
    var crm_data_id=window.localStorage.getItem('crm_data_id');
    if(crm_data_id !== '') {
        CRMDataID = crm_data_id;
        console.log(" dialing set crm data");
    }
	...
	localStorage.setItem("NumberToDial", number_dial);
    /** Load crm header data */
    var crm_skill=window.localStorage.getItem('skill');
    console.log("crm_skill lebgh= "+crm_skill.length);
    ...
    crm_data_id=getLocalVariable("CRMDataID");
    $('#dial_crm_number').val(number_dial);
    if(getLocalVariable("CALL_TYPE") != 'RD')
    {
        $('#crm_phone_number').html(getLocalVariable('isDisplayPhone') === 'false' || getLocalVariable('isDisplayPhone') === false ? 'XXXXXXXXXX<img src="assets/images/call.svg" width="20" height="20" id="crm_redial" ondblclick=DialByLine("audio","","'+number_dial+'","","","RD","'+crm_data_id+'")>' : number_dial+'<img src="assets/images/call.svg" width="20" height="20" id="crm_redial" ondblclick=DialByLine("audio","","'+number_dial+'","","","RD","'+crm_data_id+'")>');
    }
    
    $('#crm_did_number').html(callerid);
    $('#crm_did_name').html(callerid);
    $('#crm_rid').html(datetime_sip+SIP_ID);
    $('#crm_skill').html(valuesArray[0]);
    var record_id=datetime_sip+SIP_ID;
    if(valuesArray.length>0)
    {
        localStorage.setItem("crm_skill", valuesArray[0]);
    }else{
        localStorage.setItem("crm_skill", ""); 
    }
    
    localStorage.setItem("RecordID", record_id);
    var webrtc_skill=window.localStorage.getItem('skill');
    var webArray = Object.keys(JSON.parse(webrtc_skill));
    if(webArray.length>0)
    {
        var strSkillDial=webArray[0];
    }else
    {
        var strSkillDial='';
    }
    ...
    console.log("extension dialing= "+number_dial.length);
    console.log("extension dialing= "+number_dial.length);
    if(number_dial.length==5)
    {
        dialledNumber='EXTCALL';
        //DialString = "EC" & NumberToDial & "-" & DispalyPhoneNumber & "-" & CampaignName & "-TC-" & TCC_RID & "-" & CRMDataID & "-1-" & DisplayCallerID & "-" & tcc_data_id & "-" & ClientID & "-" & USER_ID
        var sip_headers_value='EC'+number_dial+'-'+SIP_ID+'-'+Campaign_name+'-EC-'+RecordID+'-'+CRMDataID+'-1-'+callerid+'-'+tcc_data_id+'-'+ClientID+'-'+USER_ID;
        var call_type='EC';
        setLocalVariable('CALL_TYPE','EC');
    }
    else if(call_type=='PG')
    {
        var sip_headers_value='PG-'+callerid+'-'+Campaign_name+'-'+RecordID+'/'+dynamic_campaign_group+'/'+AgentPrefix+'/'+Dynamic_Campaign_prefix+number_dial+'-'+CRMDataID+'-'+ClientID+'-'+CustomerTableID+'-'+CRMLeadId+'-'+BillDetails+'-'+strSkillDial;
        setLocalVariable('CALL_TYPE',call_type);
    }
    else if(call_type=='RD')
    {
        var sip_headers_value='RD-'+callerid+'-'+Campaign_name+'-'+RecordID+'/'+dynamic_campaign_group+'/'+AgentPrefix+'/'+Dynamic_Campaign_prefix+number_dial+'-'+CRMDataID+'-'+ClientID+'-'+CustomerTableID+'-'+CRMLeadId+'-'+BillDetails+'-'+strSkillDial;
        setLocalVariable('CALL_TYPE',call_type);
    }
    else if(call_type=='PV')
    {
        var sip_headers_value='PV-'+callerid+'-'+Campaign_name+'-'+RecordID+'/'+dynamic_campaign_group+'/'+AgentPrefix+'/'+Dynamic_Campaign_prefix+number_dial+'-'+CRMDataID+'-'+ClientID+'-'+CustomerTableID+'-'+CRMLeadId+'-'+BillDetails+'-'+strSkillDial;
        setLocalVariable('CALL_TYPE',call_type);
    }
    else
    {
        var call_type='MC';
        var sip_headers_value='MC-'+callerid+'-'+Campaign_name+'-'+RecordID+'/'+dynamic_campaign_group+'/'+AgentPrefix+'/'+Dynamic_Campaign_prefix+number_dial+'-'+CRMDataID+'-'+ClientID+'-'+CustomerTableID+'-'+CRMLeadId+'-'+BillDetails+'-'+strSkillDial;
        setLocalVariable('CALL_TYPE','MC');
    }
    if(call_type != '')
    {
        setLocalVariable('CALL_TYPE',call_type); 
    }
    console.log("extension dialing string= "+sip_headers_value);
    /** Enter log for Dilar */
    logEntry("Audio Call=  "+sip_headers_value+" Call type= "+call_type);
    /** end log */
	var urlmappedtodid = JSON.parse(getLocalVariable('Allow_DID_Map_URL'));
	
	if (getLocalVariable('Campaign_Crm_Mode') == '1' && getLocalVariable('WebScript').length > 0  && getLocalVariable('DisplayCallerID').length > 0 && getLocalVariable('Campaign_Crm_Status') == '1') {
		openExternalCRM('', record_id, '', number_dial);
	}else if (getLocalVariable('Campaign_Crm_Mode') == '1' && getLocalVariable('WebScript').length > 0  && getLocalVariable('DisplayCallerID').length > 0 && getLocalVariable('Campaign_Crm_Status') == '0') {
		openExternalCRM('', record_id, '', number_dial);
	}else{
		
        if(call_type=='RD')
        {
            var LastDialNumber=getLocalVariable('LastDialNumber');
            //webrtc_crm(record_id,LastDialNumber,call_type);
            insert_disposition(record_id,call_type,number_dial,callerid);
        }else
        {
            webrtc_crm(record_id,number_dial,call_type);
            insert_disposition(record_id,call_type,number_dial,callerid);
        }
	}
    /* Dialer value*/
        ...
        try{
            ...
            for (const [key, value] of Object.entries(inviteExtraHeaders)) {
                if(value == ""){
                    ...
                    spdOptions.extraHeaders.push(key + ": "+  value);
					//console.log("Extra header value= "+value+ " Key= "+key);
                }
            }
        } catch(e){}
    }
    ...
    var targetURI = SIP.UserAgent.makeURI("sip:" + dialledNumber.replace(/#/g, "%23") + "@" + SipDomain);
	//webrtc calling process//
    console.log(" webrtc call userAgent= ",userAgent);
    console.log(" webrtc call targetURI= ",targetURI);
    console.log(" webrtc call spdOptions= ",spdOptions);
    lineObj.SipSession = new SIP.Inviter(userAgent, targetURI, spdOptions);
    ...
    lineObj.SipSession.delegate = {
        onBye: function(sip){
            onSessionReceivedBye(lineObj, sip);
            console.log("webrtc session receive bye..!");
        },
        onMessage: function(sip){
            onSessionReceivedMessage(lineObj, sip);
            console.log("webrtc session onSessionReceivedMessage..!");
        },
        onInvite: function(sip){
            onSessionReinvited(lineObj, sip);
            console.log("webrtc session onSessionReinvited..!");
        },
        onSessionDescriptionHandler: function(sdh, provisional){
            onSessionDescriptionHandlerCreated(lineObj, sdh, provisional, false);
            console.log("webrtc session onSessionDescriptionHandlerCreated..!");
        }
    }
    var inviterOptions = {
        requestDelegate: { // OutgoingRequestDelegate
            onTrying: function(sip){
                onInviteTrying(lineObj, sip);
                console.log("webrtc session onInviteTrying..!");
                 logEntry("Dial Number "+number_dial+" Call Trying");
            },
            onProgress:function(sip){
                onInviteProgress(lineObj, sip);
                console.log("webrtc session onInviteProgress..!");
                logEntry("Dial Number "+number_dial+" Call  InProgress..!");
            },
            onRedirect:function(sip){
                onInviteRedirected(lineObj, sip);
                console.log("webrtc session onInviteRedirected..!");
                logEntry("Dial Number "+number_dial+" Call  Redirected..!");
            },
            onAccept:function(sip){
                onInviteAccepted(lineObj, false, sip);
                console.log("webrtc session onInviteAccepted..!");
                logEntry("Dial Number "+number_dial+" Call Accepted");
            },
            onReject:function(sip){
                onInviteRejected(lineObj, sip);
                console.log("webrtc session onInviteRejected..!");
                logEntry("Dial Number "+number_dial+" Call Rejected");
            }
        }
    }
    lineObj.SipSession.invite(inviterOptions).catch(function(e){
...
function ReviewCallMode(number_dial='') {

    alertify.success("Review mode on "+number_dial);
    call_type='';
    if (websocketConnected == 0) {
        alertify.error("Control server connection failed.");
        return;
	...
	/************Hold Time review Duration*******/
    localStorage.setItem("webrtc_review_time",getCurrentDateTime());
    var call_review_count=parseInt(window.localStorage.getItem('Review_Count'));
    var call_review_count=(call_review_count+1);
    localStorage.setItem("Review_Count", call_review_count);
...
function AudioCallRecall(lineObj, dialledNumber, extraHeaders, number_dial='', call_type='') {
    if (websocketConnected == 0) {
        alertify.error("Control server connection failed.");
        return;
    }
    ...
    $('#top-crm-continer2').css("display", "none");
    $('#top-crm-continer3').css("display", "none");
    var recall_count=parseInt(window.localStorage.getItem('Recall_Count'));
    var recall_count=(recall_count+1);
    localStorage.setItem("Recall_Count", recall_count);
    localStorage.setItem("num_of_Recall", recall_count);
    localStorage.setItem("WaitingDisposition",'true');
    localStorage.setItem("pre_callback_dialling",1);
	/*SIp Calling*/
	datetime_sip=formatDate();
	var Campaign_name=window.localStorage.getItem('CampaignName');
	var callerid=window.localStorage.getItem('ManualCallerID');
    ...
    crm_data_id=getLocalVariable("CRMDataID");
    $('#dial_crm_number').val(number_dial);
    $('#crm_phone_number').html(getLocalVariable('isDisplayPhone') === 'false' || getLocalVariable('isDisplayPhone') === false ? 'XXXXXXXXXX<img src="assets/images/call.svg" width="20" height="20" id="crm_redial" ondblclick=DialByLine("audio","","'+number_dial+'","","","RD","'+crm_data_id+'")>' : number_dial+'<img src="assets/images/call.svg" width="20" height="20" id="crm_redial" onclick=ondblclick("audio","","'+number_dial+'","","","RD","'+crm_data_id+'")>');
    $('#crm_did_number').html(callerid);
    $('#crm_did_name').html(callerid);
    $('#crm_rid').html(datetime_sip+SIP_ID);
    $('#crm_skill').html(valuesArray[0]);
    ...
    {
        var call_type='RC';
        var sip_headers_value='RC-'+callerid+'-'+Campaign_name+'-'+RecordID+'/'+dynamic_campaign_group+'/'+AgentPrefix+'/'+Dynamic_Campaign_prefix+number_dial+'-'+CRMDataID+'-'+ClientID+'-'+CustomerTableID+'-'+CRMLeadId+'-'+BillDetails+'-'+strSkillDial;
        setLocalVariable('CALL_TYPE','RC');
    }
    console.log("extension dialing string= "+sip_headers_value);
    /** Enter log for Dilar */
    logEntry("Audio Recal Call=  "+sip_headers_value+" Call type= "+call_type);
    /** end log */
	var urlmappedtodid = JSON.parse(getLocalVariable('Allow_DID_Map_URL'));
...
function MO_AudioCall(lineObj, dialledNumber, extraHeaders) {
    if (websocketConnected == 0) {
        alertify.error("Control server connection failed.");
        return;
    }
...
function MODialing(type, buddy, numToDial, CallerID, extraHeaders, mo_type='', MoAgentName=''){
    if (websocketConnected == 0) {
        alertify.error("Control server connection failed.");
        return;
    }
    ...
    var set_number_string="";
    if(mo_type=='BARGE')
    {
        set_number_string=MoAgentName+"!BARGE";
        setLocalVariable('NumberToDial', set_number_string);
    }
    if(mo_type=='COACH')
    {
        set_number_string=MoAgentName+"!COACH";
        setLocalVariable('NumberToDial', set_number_string);
    }
    if(mo_type=='INFO')
    {
        set_number_string=MoAgentName+"!INFO";
        setLocalVariable('NumberToDial', set_number_string);
    }
    if(mo_type=='CONF')
    {
        set_number_string=MoAgentName+"!TL CONF";
        setLocalVariable('NumberToDial', set_number_string);
    }
         /** Enter log for Dilar */
        logEntry("MODialing Call "+numToDial+" Mo Name"+set_number_string);
        /** end log */
    ...
    // Start Call Invite
    if(type == "audio"){
        MO_AudioCall(lineObj, numDial, extraHeaders);
    } 
    else {
...
function StartTransferSession(lineNum){
    ...
    /** Enter log for Dilar */
    logEntry("Start Transfer Session Call "+lineNum);
    /** end log */
    ...
    localStorage.setItem("isLineBusy",'TRANSFER');
...
function CancelTransferSession(lineNum){
    /** Enter log for Dilar */
    logEntry("Cancel Transfer Call "+lineNum);
    /** end log */
    var lineObj = FindLineByNumber(lineNum);
...
async function BlindTransfer(lineNum, dial_number='') {
    if (websocketConnected == 0) {
        alertify.error("Control server connection failed.");
        return;
    }
    
    /*** start transfer counter */
    let tc_count = parseInt(window.localStorage.getItem('Transfer_Count'));
    let tc_counter = tc_count + 1;
    localStorage.setItem("Transfer_Count", tc_counter);
    /****Transfer Count data****/
    ...
    /** end transfer counter */
    if(dial_number !='')
    {
        var dstNo = dial_number;
    }else
    {
        var dstNo = $("#line-" + lineNum + "-txt-FindTransferBuddy").val();
    }
    
    var ExternalTransfer=getLocalVariable('ExternalTransfer');
    var InternalTransfer=getLocalVariable('InternalTransfer');
    if (dstNo.length==5)
        {
            if(InternalTransfer != '1')
            {
                alertify.error("Permission denied for internal blind transfers.");
                logEntry("Permission denied for internal blind transfers.");
                return false;
            }
        }else
        {
            ...
            {
                alertify.error("Permission denied for external blind transfers.");
                logEntry("Permission denied for external blind transfers.");
                return false;
            }
        }


    let number_dial = dstNo;
    ...
    let lineObj = FindLineByNumber(lineNum);
    if (lineObj === null || lineObj.SipSession === null) {
        console.warn("Null line or session");
        return;
    }
    ...
    //console.log("transfer value123 ",transferOptions);
    //console.log("transfer value1234 "+JSON.stringify(transferOptions));

    /* SIP Calling */
    let datetime_sip = formatDate();
    let Campaign_name = window.localStorage.getItem('CampaignName');
    let callerid = window.localStorage.getItem('ManualCallerID');
    let SIP_ID = window.localStorage.getItem('SIP_ID');
    let RecordID = datetime_sip + SIP_ID + "-SIP";
    let dynamic_campaign_group = window.localStorage.getItem('Campaign_Group');
    // var AgentPrefix = window.localStorage.getItem('agentPrefix');
    let AgentPrefix = '';
    let Dynamic_Campaign_prefix = window.localStorage.getItem('Campaign_PreFix');
    if (number_dial.length == 5) {
        Dynamic_Campaign_prefix = '';
    }
    let crm_data_id = window.localStorage.getItem('crm_data_id');
    let CRMDataID = 1;
    if (crm_data_id !== '') {
        CRMDataID = crm_data_id;
    }
    let ClientID = window.localStorage.getItem('ClientID');
    let CustomerTableID = 0;
    let CRMLeadId = Campaign_name + "_MANUAL";
    let BillDetails = '0~0/0';
    let webrtc_skill = window.localStorage.getItem('skill');
    let webArray = Object.keys(JSON.parse(webrtc_skill));
    let strSkillDial = webArray.length > 0 ? webArray[0] : '';
    
    localStorage.setItem("NumberToDial2", number_dial);
    localStorage.setItem("isLineBusy", 'TRANSFER');
    localStorage.setItem("isTransfer", '1');
    
    /** insert data in database */
    let to_Number = number_dial;
    let cc_CallType = 'TC';
    let Main_Call_Number = window.localStorage.getItem('NumberToDial');
    let CC_Type = to_Number.length == 5 ? 'IT' : 'ET';
    let to_Campaign = '';
    let tcc_data_id = '';
    let LoginHourID = window.localStorage.getItem('LoginHourID');
    let ModeDBId = window.localStorage.getItem('ModeDBId');
    let USER_ID = window.localStorage.getItem('USER_ID');
    let CampaignName = window.localStorage.getItem('CampaignName');
    let DisplayCallerID = window.localStorage.getItem('ManualCallerID');
    let cc_defined_CallType = 'BT';
    let Call_RID = datetime_sip + SIP_ID;
    
    let jsonData = {
        to_Number: to_Number,
        cc_CallType: cc_CallType,
        Call_RID: Call_RID,
        Main_Call_Number: Main_Call_Number,
        CC_Type: CC_Type,
        to_Campaign: to_Campaign,
        tccRID: Call_RID,
        ClientID: ClientID,
        LoginHourID: LoginHourID,
        ModeDBId: ModeDBId,
        USER_ID: USER_ID,
        CampaignName: CampaignName,
        DisplayCallerID: DisplayCallerID,
        cc_defined_CallType: cc_defined_CallType
    };
    
    try {
        let response = await fetch('/api_calling/save_tcc_details', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        let data = await response.json();
        if (data.status == 1) {
            console.log("insert transfer " + data.tc_id);
            tcc_data_id = data.tc_id;
        }
    } catch (error) {
        console.error('Error:', error);
    }
    ...
    if (to_Number.length==5)
        {
            var new_RecordID = datetime_sip + SIP_ID;
            var newsipheader='EC'+number_dial+'-'+Main_Call_Number+'-'+Campaign_name+'-TC-'+new_RecordID+'-'+CRMDataID+'-1-'+callerid+'-'+tcc_data_id+'-'+ClientID+'-'+USER_ID;
            //var newsipheader=number_dial+'-'+Main_Call_Number+'-'+Campaign_name+'-TC-'+RecordID+'-'+CRMDataID+'-1-'+callerid+'-'+tcc_data_id+'-'+ClientID+'-'+USER_ID;
            //20015-9873983318-testwebrtc-TC-24073117231671620014-265-1-01168180030-3809084-3181-abhi
        }else
        {
            var newsipheader='TC-'+callerid+'-'+Campaign_name+'-'+RecordID+'/'+dynamic_campaign_group+'/'+Dynamic_Campaign_prefix+'/'+number_dial+'-'+CRMDataID+'-'+ClientID+'-'+CustomerTableID+'-'+CRMLeadId+'-'+BillDetails+'-'+strSkillDial;
            
        }
        console.log("Blind Transfer Dialstring= "+newsipheader);
        //transferOptions.extraHeaders.push('dcgalaxy' + ": "+newsipheader);
        console.log("REFER: ", newsipheader + "@" + SipDomain);
        var referTo = SIP.UserAgent.makeURI("sip:"+ newsipheader.replace(/#/g, "%23") + "@" + SipDomain);
    try {
        await session.refer(referTo, transferOptions);
        console.log("transfer ses",session);
...
/**** Queue Transfer *** */
async function QueueTransfer(lineNum, dial_number='',tcamp,tpriority,tqueue) {
    if (websocketConnected == 0) {
        alertify.error("Control server connection failed.");
        return;
    }
    ...
    /** Enter log for Dilar */
    logEntry("Queue Transfer Call " + dial_number);
    /** end log */
    /*** start transfer counter */
    let tc_count = parseInt(window.localStorage.getItem('Transfer_Count'));
    let tc_counter = tc_count + 1;
    ...
    var transferOptions = {
        requestDelegate: {
            onAccept: function (sip) {
                console.log("Queue transfer Accepted");
                /******unhold session***** */
                logEntry("Queue transfer Accepted");	
                /******unhold session***** */							
                ...
                // TODO: use lang pack
                $("#line-" + lineNum + "-msg").html("Call Queue Transferred (Accepted)");

                updateLineScroll(lineNum);

                session.bye().catch(function (error) {
                    console.warn("Could not BYE after Queue transfer:", error);
                });
                teardownSession(lineObj);
            },
            onReject: function (sip) {
                ...
                $("#line-" + lineNum + "-msg").html("Call Queue Failed!");
    ...
    let Campaign_name = window.localStorage.getItem('CampaignName');
    let SIP_ID = window.localStorage.getItem('SIP_ID');
    let RecordID = datetime_sip + SIP_ID;
    let crm_data_id = window.localStorage.getItem('crm_data_id');
    var CRMDataID = 1;
    if (crm_data_id !== '') {
        CRMDataID = crm_data_id;
    ...
    /** insert data in database */
    let cc_CallType = window.localStorage.getItem('CALL_TYPE');
    let Main_Call_Number = window.localStorage.getItem('NumberToDial');
    let CC_Type ='QT';
    let to_Campaign = tcamp;
    let tcc_data_id = '';
    let LoginHourID = window.localStorage.getItem('LoginHourID');
    let ModeDBId = window.localStorage.getItem('ModeDBId');
    let USER_ID = window.localStorage.getItem('USER_ID');
    ...
    /** END insert data in database */
    IVRInput='';
    //var newsipheader = "GCT"+tcamp+"_"+tskill+"-"+DisplayCallerID+"-"+Campaign_name+"-TQ-"+RecordID+"-"+tcc_data_id+"-"+IVRInput+"-"+number_dial+"---"+USER_ID+"-"+ClientID+"-"+CustomerTableID+"-"+CRMLeadId;    
    //GCTtestwebrtc-01168180030-demoptl-TQ-24072316113273220015-3808975--8285248951-0-web1-alok-3181-0-DEMOPTL_MANUAL
   // DialString = "GCT" & cmbCampaign.Text & "-" & DisplayCallerID & "-" & CampaignName & "-TQ-" & TCC_RID & "-" & tcc_data_id & "-" & IVRInput & "-" & DispalyPhoneNumber & "-" & getTransQueuePriority(cmbTransferQueue) & "-" & cmbTransferQueue.Text & "-" & USER_ID & "-" & ClientID & "-" & CustomerTableID & "-" & CRMLeadId
//Result = GDN.TransferCallEx(0, DialString)
    newsipheader = "GCT"+tcamp+"-"+DisplayCallerID+"-"+CampaignName+"-TQ-"+RecordID+"-"+tcc_data_id+"-"+IVRInput+"-"+number_dial+"-"+tpriority+"-"+tqueue+"-"+USER_ID+"-"+ClientID+"-"+CustomerTableID+"-"+CRMLeadId;
    console.log("queue Transfer Dialstring= "+newsipheader);
        //transferOptions.extraHeaders.push('dcgalaxy' + ": "+newsipheader);
        console.log("REFER: ", newsipheader + "@" + SipDomain);
        var referTo = SIP.UserAgent.makeURI("sip:"+ newsipheader.replace(/#/g, "%23") + "@" + SipDomain);
    try {
...
/**** Skill Transfer *** */
async function skillTransfer(lineNum, dial_number='',tcamp,tskill) {
    ...
    /** Enter log for Dilar */
    logEntry("Skill Transfer Call " + dial_number);
    /** end log */
    /*** start transfer counter */
    let tc_count = parseInt(window.localStorage.getItem('Transfer_Count'));
    let tc_counter = tc_count + 1;
    ...
    var transferOptions = {
        requestDelegate: {
            onAccept: function (sip) {
                console.log("Skill transfer Accepted");
                /******unhold session***** */								
                ...
                // TODO: use lang pack
                $("#line-" + lineNum + "-msg").html("Call Skill Transferred (Accepted)");

                updateLineScroll(lineNum);

                session.bye().catch(function (error) {
                    console.warn("Could not BYE after Skill transfer:", error);
                });
                teardownSession(lineObj);
            },
            onReject: function (sip) {
                ...
                $("#line-" + lineNum + "-msg").html("Call Skill Failed!");
    ...
    IVRInput='';
    var newsipheader = "GCT"+tcamp+"_"+tskill+"-"+DisplayCallerID+"-"+Campaign_name+"-TQ-"+RecordID+"-"+tcc_data_id+"-"+IVRInput+"-"+number_dial+"---"+USER_ID+"-"+ClientID+"-"+CustomerTableID+"-"+CRMLeadId;    
    console.log("Skill Transfer Dialstring= "+newsipheader);
        //transferOptions.extraHeaders.push('dcgalaxy' + ": "+newsipheader);
        console.log("REFER: ", newsipheader + "@" + SipDomain);
        var referTo = SIP.UserAgent.makeURI("sip:"+ newsipheader.replace(/#/g, "%23") + "@" + SipDomain);
    try {
...
/****End Feedback transfer */

/*** Campaign Transfer */

async function CampaignTransfer() {
    var dialline = parseInt(window.localStorage.getItem('Webrtc_dialline'));
    var isLineBusy = window.localStorage.getItem('isLineBusy');
    var transfer_campaign = $("#dialpad_campaign").val();
    let CampaignName = window.localStorage.getItem('CampaignName');
    let ExternalTransfer=getLocalVariable('ExternalTransfer');
    let InternalTransfer=getLocalVariable('InternalTransfer');
    // SIP Calling
    var datetime_sip = formatDate();
            
    // Insert data into the database
    var ClientID = window.localStorage.getItem('ClientID');
    var caller_id = getLocalVariable('ManualCallerID');
    var DispalyPhoneNumber = getLocalVariable('NumberToDial');
    ...
    let LoginHourID = window.localStorage.getItem('LoginHourID');
    let ModeDBId = window.localStorage.getItem('ModeDBId');
    var CRMLeadId = CampaignName + "_MANUAL";
    let DisplayCallerID = window.localStorage.getItem('ManualCallerID');
    let cc_defined_CallType = 'CT';
    let SIP_ID = window.localStorage.getItem('SIP_ID');
    let Call_RID = datetime_sip + SIP_ID;
    let IVRInput = 0;
    let CustomerTableID = 0;
    if (!(getLocalVariable('isLineBusy') == 'ONCALL' || getLocalVariable('isLineBusy') == 'HOLD' || getLocalVariable('isLineBusy') == 'MUTE')) {
        ...
        logEntry("No Active Calls for Campaign Transfer");
        return false;
    }
    /***permission */
    let response = await fetch('/campaign_transfer_permission', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
            client_id: client_id,
            from_campaign: CampaignName,
            to_campaign: transfer_campaign
        })
    });
    let cam_result = await response.json();
    if (cam_result.status === false || cam_result.status == 'false') {
        logEntry("Campaign Not Transfer");
        alertify.error("Campaign Permission denied...!");
        return false;
    }
    ...
    if (ExternalTransfer==0 && InternalTransfer==0) {
        alertify.error("Permission denied...!");
        return false;
    }
    if (transfer_campaign=='') {
        alertify.error("Please Select Campaign To Transfer...!");
        return false;
    }
    if (transfer_campaign==CampaignName) {
        alertify.error("Same Campaign Transfer Not Allowed...!");
        return false;
    }

    alertify.confirm("Are you sure you want to Campaign Transfer ("+transfer_campaign+") ?", async function (e) {
        if (e) {
            let jsonData = {
                to_Number: transfer_campaign,
                cc_CallType: cc_CallType,
                Call_RID: getLocalVariable('RecordID'),
                Main_Call_Number: Main_Call_Number,
                CC_Type: CC_Type,
                to_Campaign: transfer_campaign,
                tccRID: Call_RID,
            ...
            console.log("Tcc Details entry"+JSON.stringify(jsonData));
            try {
                let response = await fetch('/api_calling/save_tcc_details', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                if (data.status === 1) {
                    console.log("Insert transfer " + data.tc_id);
                    alertify.success("Insert transfer= " + data.tc_id);
                    tcc_data_id = data.tc_id;
                }
            } catch (error) {
                console.error('Error:', error);
            ...
            // Enter log for Dialer
            var lineNum = dialline;
            var NumberToDial = getLocalVariable('NumberToDial');
            
            if (NumberToDial !== null && NumberToDial !== '') {
                logEntry("Campaign Transfer Call " + NumberToDial);
            }
            var lineObj = FindLineByNumber(lineNum);
            var dstNo = 'GALAXY';
            var session = lineObj.SipSession;
            console.log("sip session= ", lineObj);
            if (!session.data.transfer) session.data.transfer = [];
            session.data.transfer.push({
                type: "Campaign",
                to: dstNo,
            ...
            var transferOptions = {
                requestDelegate: {
                    onAccept: function (sip) {
                        console.log("Campaign Transfer Accepted");
                        logEntry("Campaign Transfer Accepted "+DispalyPhoneNumber);
                        session.data.terminateby = "us";
                        session.data.reasonCode = 202;
                        session.data.reasonText = "Transfer";
                        ...
                        $("#line-" + lineNum + "-msg").html("Call Feedback Transferred (Accepted)");

                        updateLineScroll(lineNum);

                        session.bye().catch(function (error) {
                            console.warn("Could not BYE after Feedback transfer:", error);
                        });
                    ...
                    onReject: function (sip) {
                        ...
                        $("#line-" + lineNum + "-msg").html("Call Blind Transfer Failed!");
            ...
            var newsipheader = 'GCT' + transfer_campaign + '-' + caller_id + '-' + CampaignName + '-TQ-' + Call_RID + '-' + tcc_data_id + '-' + IVRInput + '-' + DispalyPhoneNumber + '-0--' + USER_ID + '-' + ClientID + '-' + CustomerTableID + '-' + CRMLeadId;
            
            console.log("Transfer value: " + newsipheader);
            var referTo = SIP.UserAgent.makeURI("sip:" + newsipheader.replace(/#/g, "%23") + "@" + SipDomain);
...
/**** Feedback Transfer *** */
async function Feedbacktransfer() {
    var dialline=parseInt(window.localStorage.getItem('Webrtc_dialline'));
    var isLineBusy=window.localStorage.getItem('isLineBusy');
    var feedback_permission=parseInt(window.localStorage.getItem('feedback_permission'));
    if (websocketConnected == 0) {
        alertify.error("Control server connection failed.");
        return false;
    }
    if(isLineBusy != 'ONCALL')
    {
            alertify.error("No Active Calls");
            logEntry("No Active Calls");
            return false;
    }
    if(feedback_permission != 1)
        {
            alertify.error("Feedback permission denied.");
            logEntry("Feedback permission denied.");
            return false;
        }

    alertify.confirm("Are you surely want to Feedback Transfer?", function (e) {
        if (e) {
        /** Enter log for Dilar */
      
      
      lineNum=dialline;
      NumberToDial=getLocalVariable('NumberToDial');
      if(NumberToDial != null || NumberToDial != '')
      {
            logEntry("Feedback Transfer Call "+NumberToDial);
      }
      ...
      var transferOptions  = { 
          requestDelegate: {
              onAccept: function(sip){
                  console.log("Feedback Transfer Accepted");
                    logEntry("Feedback Transfer Accepted ");
                  session.data.terminateby = "us";
                  session.data.reasonCode = 202;
                  session.data.reasonText = "Transfer";
      ...
      var CALL_TYPE=window.localStorage.getItem('CALL_TYPE');
      var ClientID=window.localStorage.getItem('ClientID');
      var CampaignName=window.localStorage.getItem('CampaignName');
  /**END insert data in database */
    var newsipheader='FB'+CALL_TYPE+ClientID+CampaignName;
      console.log("transfer value"+newsipheader);
      //transferOptions.extraHeaders.push('dcgalaxy' + ": "+newsipheader);
      console.log("REFER: ", newsipheader + "@" + SipDomain);
      var referTo = SIP.UserAgent.makeURI("sip:"+ newsipheader.replace(/#/g, "%23") + "@" + SipDomain);
...
/****End Feedback transfer */
async function AttendedTransfer(lineNum, dial_number=''){
    //alert("hello");
    ...
    logEntry("Attended Transfer Call "+lineNum);
    /** end log */
    var tc_count=parseInt(window.localStorage.getItem('Conference_Count'));
    var tc_counter=tc_count+1;
    localStorage.setItem("Conference_Count", tc_counter);
    /******Update conference Transfer data***** */
    update_confdata(tc_counter);
    ...
    /*** Dial Permission */
    var ExternalConference=getLocalVariable('ExternalConference');
    var InternalConference=getLocalVariable('InternalConference');
    if (dstNo.length==5)
    {
        if(InternalConference != '1')
        {
            alertify.error("Permission denied for internal attended transfers.");
            logEntry("Permission denied for internal attended transfers.");
            return false;
        }
    }else
    {
        if(ExternalConference != '1')
        {
            alertify.error("Permission denied for external attended transfers.");
            logEntry("Permission denied for external attended transfers.");
            return false;
        }
    }
    /*** end Dial Permisiion */
	var number_dial=dstNo;
    if(EnableAlphanumericDial){
        dstNo = dstNo.replace(telAlphanumericRegEx, "").substring(0,MaxDidLength);
    }
    else {
    ...
    /**insert data in database */
        var to_Number=window.localStorage.getItem('NumberToDial2');
        var cc_CallType='CC';
        var Main_Call_Number=window.localStorage.getItem('NumberToDial');
        if (to_Number.length==5)
        {
            var CC_Type='IT';
        }else
        ...
        var to_Campaign='';
        var LoginHourID=window.localStorage.getItem('LoginHourID');
        var ModeDBId=window.localStorage.getItem('ModeDBId');
        var USER_ID=window.localStorage.getItem('USER_ID');
        var CampaignName=window.localStorage.getItem('CampaignName');
        var DisplayCallerID=window.localStorage.getItem('ManualCallerID');
        var cc_defined_CallType='AT';
        var Call_RID=datetime_sip+SIP_ID;
        var jsonData={to_Number:to_Number,cc_CallType:cc_CallType,Call_RID:Call_RID,Main_Call_Number:Main_Call_Number,CC_Type:CC_Type,to_Campaign:to_Campaign,tccRID:Call_RID,ClientID:ClientID,LoginHourID:LoginHourID,ModeDBId:ModeDBId,USER_ID:USER_ID,CampaignName:CampaignName,DisplayCallerID:DisplayCallerID,cc_defined_CallType:cc_defined_CallType
        };

        console.log("save_tcc_details Ad= "+JSON.stringify(jsonData));
        try {
            let response = await fetch('/api_calling/save_tcc_details', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    if (to_Number.length==5)
    {
        dialledNumber='EXTCALL';
        var new_RecordID=datetime_sip+SIP_ID;
        //var newsipheader='EC'+number_dial+'-'+SIP_ID+'-'+Campaign_name+'-EC-'+RecordID+'-'+CRMDataID+'-1-'+callerid+'-'+tcc_data_id+'-'+ClientID+'-'+USER_ID;
        var newsipheader='EC'+number_dial+'-'+Main_Call_Number+'-'+Campaign_name+'-CC-'+new_RecordID+'-'+CRMDataID+'-1-'+callerid+'-'+tcc_data_id+'-'+ClientID;
        spdOptions.extraHeaders.push('dcgalaxy' + ": "+newsipheader);
    }else
    {
        dialledNumber='GALAXY';
        var newsipheader='CC-'+callerid+'-'+Campaign_name+'-'+RecordID+'/'+dynamic_campaign_group+'/'+AgentPrefix+'/'+Dynamic_Campaign_prefix+number_dial+'-'+CRMDataID+'-'+ClientID+'-'+CustomerTableID+'-'+CRMLeadId+'-'+BillDetails+'-'+strSkillDial;
        spdOptions.extraHeaders.push('dcgalaxy' + ": "+newsipheader);
    }
	
	
	/*end dial string*/
    console.log("TRANSFER INVITE: ", "sip:" + dialledNumber + "@" + SipDomain);
    var targetURI = SIP.UserAgent.makeURI("sip:"+ dialledNumber.replace(/#/g, "%23") + "@" + SipDomain);
    ...
    var inviterOptions = {
        requestDelegate: {
            ...
            onAccept:function(sip){
                ...
                var agent_CompleteTransferBtn = $("#agent_completeattended_transfer");
                ...
                agent_CompleteTransferBtn.on('click', function(){
                    //alert("Complete Transfer");
                    var transferOptions  = { 
                        requestDelegate: {
                            onAccept: function(sip){
                                console.log("Attended transfer Accepted");
...
function StartConferenceCall(lineNum){
    ...
    /** Enter log for Dilar */
    logEntry("Start Conference Call "+lineNum);
    /** end log */
...
function CancelConference(lineNum){
    ...
    /** Enter log for Dilar */
    logEntry("Conference Cancel "+lineNum);
    /** end log */
    var lineObj = FindLineByNumber(lineNum);
    if(lineObj == null || lineObj.SipSession == null){
        console.warn("Null line or session");
...
async function ConferenceDial(lineNum){
    ...
    if (dstNo.length==5)
    {
        if(InternalConference != '1')
        {
            alertify.error("Permission denied for internal conference call.");
            logEntry("Permission denied for internal conference call.");
            return false;
        }
    }else
    {
        if(ExternalConference != '1')
        {
            alertify.error("Permission denied for external conference call.");
            logEntry("Permission denied for external conference call.");
            return false;
        }
    }
    /*** end Dial Permisiion */

     /*** start conference counter */
     var cc_count=parseInt(window.localStorage.getItem('Conference_Count'));
     var cc_counter=cc_count+1;
     localStorage.setItem("Conference_Count", cc_counter);
     /******Update conference Transfer data***** */
    update_confdata(cc_counter);
     /** end conference counter */
	var number_dial=dstNo;
	console.log("webrtc conference dial"+dstNo);

    /** Enter log for Dilar */
    logEntry("Conference Call "+number_dial);
    /** end log */
    ...
    var session = lineObj.SipSession;
	console.log("webrtc conf session= ",session);
	console.log("webrtc conf lineObj= ",lineObj);
	...
	console.log("webrtc spdOptions= ",spdOptions);
	/*SIp Calling*/
	datetime_sip=formatDate();
	var Campaign_name=window.localStorage.getItem('CampaignName');
	var callerid=window.localStorage.getItem('ManualCallerID');
	...
	{
		var strSkillDial=skillkeys[0];
	}*/
	
	localStorage.setItem("NumberToDial2", number_dial);
    localStorage.setItem("isLineBusy",'CONFERENCE');
    localStorage.setItem("isConference",'1');
    /** Load crm header data */
    var crm_skill=window.localStorage.getItem('skill');
    var valuesArray = Object.keys(JSON.parse(crm_skill));
    crm_data_id=getLocalVariable("CRMDataID");
    $('#dial_crm_number').val(number_dial);
    $('#crm_phone_number').html(getLocalVariable('isDisplayPhone') === 'false' || getLocalVariable('isDisplayPhone') === false ? 'XXXXXXXXXX<img src="assets/images/call.svg" width="20" height="20" id="crm_redial" ondblclick=DialByLine("audio","","'+number_dial+'","","","RD","'+crm_data_id+'")>' : number_dial+'<img src="assets/images/call.svg" width="20" height="20" id="crm_redial" ondblclick=DialByLine("audio","","'+number_dial+'","","","RD","'+crm_data_id+'")>');
    $('#crm_did_number').html(callerid);
    ...
    var strSkillDial=valuesArray[0];
	///set dial number data//


    /** starts database entry */
     /**insert data in database */
     var to_Number=window.localStorage.getItem('NumberToDial2');
     var cc_CallType='CC';
     var Main_Call_Number=window.localStorage.getItem('NumberToDial');
     ...
     var CampaignName=window.localStorage.getItem('CampaignName');
     var DisplayCallerID=window.localStorage.getItem('ManualCallerID');
     var cc_defined_CallType='CC';
     var Call_RID=datetime_sip+SIP_ID;
     var tcc_data_id='';
     var jsonData={to_Number:to_Number,cc_CallType:cc_CallType,Call_RID:Call_RID,Main_Call_Number:Main_Call_Number,CC_Type:CC_Type,to_Campaign:to_Campaign,tccRID:Call_RID,ClientID:ClientID,LoginHourID:LoginHourID,ModeDBId:ModeDBId,USER_ID:USER_ID,CampaignName:CampaignName,DisplayCallerID:DisplayCallerID,cc_defined_CallType:cc_defined_CallType
     };

     console.log("Conference call json "+JSON.stringify(jsonData));
     try {
        let response = await fetch('/api_calling/save_tcc_details', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function cancelSession(lineNum) {
    ...
    if(lineNum=='cs_call')
    {
        lineNum=window.localStorage.getItem('Webrtc_dialline');
    }
    /** Enter log for Dilar */
    var NumberToDial=window.localStorage.getItem('NumberToDial');
    if(NumberToDial != '' || NumberToDial != "undefined")
    {
        logEntry("Cancel Call "+NumberToDial);
    }
    /** end log */
    var lineObj = FindLineByNumber(lineNum);
    if(lineObj == null || lineObj.SipSession == null) return;
...
function holdSession(lineNum) {
    ...
    /**Set socket data */
    if (getLocalVariable('Hold') == 0 || getLocalVariable('Hold') != '1') {
    ...
    if(NumberToDial != '' || NumberToDial != "undefined")
    {
        logEntry("Hold Call "+NumberToDial);
    }
    /** end log */

     if(getLocalVariable('CALL_TYPE') != 'TC' && getLocalVariable('CALL_TYPE') != 'QT')
     {
        localStorage.setItem("isHold",'1');
        localStorage.setItem("isLineBusy",'HOLD');
        localStorage.setItem("webrtc_hold_time",getCurrentDateTime());
        var call_hold_count=parseInt(window.localStorage.getItem('Hold_Count'));
        var call_hold_count=(call_hold_count+1);
        localStorage.setItem("Hold_Count", call_hold_count);
     }
...
function unholdSession(lineNum) {
    ...
    if(getLocalVariable('CALL_TYPE') != 'TC' && getLocalVariable('CALL_TYPE') != 'QT')
    {
        let hold_date_time = getLocalVariable('webrtc_hold_time');
        let holddate = new Date(hold_date_time);
        let currentDate = new Date();
        if(hold_date_time != null && holddate != 'Invalid Date')
        {
            console.log("hold_date_time= "+hold_date_time);
            logEntry("hold_date_time "+hold_date_time);
            logEntry("hold from time= "+holddate+" hold To time= "+currentDate);
            let differenceInSeconds = TimeDifferenceInSeconds(holddate, currentDate);
            console.log("hold_date_time differenceInSeconds= "+differenceInSeconds);
            var call_hold_count=parseInt(window.localStorage.getItem('Hold_Counter'));
            var call_hold_counter=(call_hold_count+differenceInSeconds);
            console.log("hold_date_time call_hold_counter= "+call_hold_counter);
            localStorage.setItem("Hold_Counter", call_hold_counter);
            logEntry("hold_date_time call_hold_counter= "+call_hold_counter);
        }
    ...
    if(NumberToDial != '' || NumberToDial != "undefined")
    {
        logEntry("Unhold Call "+NumberToDial);
    }
    /** end log */
    localStorage.setItem("isHold",'0');
    localStorage.setItem("isLineBusy",'ONCALL');
    //localStorage.setItem("hold_time","");
...
function MuteSession(lineNum){
    ...
    if (getLocalVariable('Mute') == 0 || getLocalVariable('Mute') != '1') {
        alertify.error("Access Denied");
        return false;
    }
    if(lineNum=='cs_call')
    ...
    if(NumberToDial != '' || NumberToDial != "undefined")
    {
        logEntry("Mute Call "+NumberToDial);
    }
    /** end log */
    localStorage.setItem("MuteCall",'1');
...
function UnmuteSession(lineNum){
    ...
    if(NumberToDial != '' || NumberToDial != "undefined")
    {
        logEntry("Unmute Call "+NumberToDial);
    }
    /** end log */
    localStorage.setItem("MuteCall",'0');
    localStorage.setItem("isLineBusy",'ONCALL');
...
function endSession(lineNum) {
    ...
    if (getLocalVariable('HangupCall') == 0 || getLocalVariable('HangupCall') != '1') {
        //alertify.error("Access Denied");
        return false;
    }
    if(lineNum=='cs_call')
    ...
    if(NumberToDial != '' || NumberToDial != "undefined")
    {
        logEntry("End Call "+NumberToDial);
    }
    
    /** end log */
    /** set defauls socket value */
    localStorage.setItem("isHold",'0');
	localStorage.setItem("MuteCall",'0');
    localStorage.setItem("isConference",'0');
...
var myModal = new bootstrap.Modal(document.getElementById('senddtmf'), {
    keyboard: false
});
myModal.show();
    ...
    var html = ""
    html += "<div>";
    html += "<table cellspacing=10 cellpadding=5 style=\"margin-left:auto; margin-right: auto\">";
...
function dialtime()
{
    console.log("dialtime");
    var client_id=getLocalVariable('ClientID');
    var campaign_name=getLocalVariable('CampaignName');
    var jsondata={client_id:client_id,campaign_name:campaign_name};
    console.log("show data= "+JSON.stringify(jsondata));
    var settings = {
        "url": "/validateDialTime",
        "method": "POST",
        "timeout": 0,
        "headers": {
          "Content-Type": "application/json"
      ...
      return $.ajax(settings).done(function (response) {
        ...
        console.log("dialtime = "+JSON.stringify(response));
        console.log("file vada= "+response.status);
        if(response.status==true)
        {
            var status_response='true';
        }else
        {
            var status_response='false';
        }
 ...
 async function DialByLine(type, buddy, numToDial, CallerID, extraHeaders, call_type='', data_id=''){
    
    //alert("dial line= "+type+" buddy= "+buddy+" numToDial= "+numToDial+" CallerID= "+CallerID+" extraHeaders= "+extraHeaders+" call_type= "+call_type+" data_id= "+data_id);
    if (websocketConnected == 0) {
        alertify.error("Control server connection failed.");
        return;
    }
    ...
    var WaitingDisposition=window.localStorage.getItem('WaitingDisposition');
    var isLineBusy=window.localStorage.getItem('isLineBusy');
	if(WaitingDisposition == 'true' && isLineBusy != 'READY' && call_type != 'RD')
    {
        alertify.error("Please dispose call");
        return false;
    }
    if(getLocalVariable('redial') != '1' && call_type == 'RD')
    {
        alertify.error("Access Denied");
        return false;
    }
    if(isLineBusy != 'HANGUP' && call_type == 'RD')
    {
        alertify.error("Redial works only in hang-up status");
        return false;
    }
    if(isLineBusy == 'ONCALL' && call_type == 'RD')
    {
        alertify.error("Agent is busy");
        return false;
    }
    if (getLocalVariable('reviewMode') == "1") {
        alertify.error("Review Mode On");
        return false;
    }
    //console.log("DialByLine= type= "+type+" buddy= "+buddy+" numToDial= "+numToDial+" CallerID= "+CallerID+" extraHeaders=  "+extraHeaders);

    var user_mode=window.localStorage.getItem('UserMode');
    var DialPad=window.localStorage.getItem('DialPad');
    var CRMDataID_web=window.localStorage.getItem('CRMDataID');
    
    var IsLicenseActive=window.localStorage.getItem('IsLicenseActive');
    console.log("Set User mode= "+user_mode+ "DIal pad= "+DialPad);
    console.log("license check= "+IsLicenseActive);
        let dialdatetime_new=dialtime();
    console.log("dialdatetime= "+dialdatetime_new.readyState);
    if(dialdatetime_new.readyState!=1 || dialdatetime_new.readyState !='1')
    {
        alertify.error("campaign date time expire..");
        return false;
    }
    if(IsLicenseActive==false || IsLicenseActive=='false')
    {
        alertify.error("No Active License");
        return false;
    }
    if(IsLicenseActive=='')
    {
        alertify.error("wait for license");
        return false;
    }
    if(user_mode != 'Auto' && user_mode != 'Manual' && user_mode != 'Preview' && user_mode != 'Progressive')
    {
        alertify.error("Access Denied");
        return false;
    }
    if(user_mode == 'Progressive' && call_type == '')
    {
        alertify.error("Calling not allowed");
        return false;
    }
    if(DialPad !='1')
    {
	...
	var numDial2 = (numToDial)? numToDial : $("#dialText").val();
    var web_data_id=getLocalVariable("CRMDataID");

    if (isNaN(numDial)) {
        alertify.error("This is not a valid number.");
        return false;
    }
    /**Start set dataid */
    if(data_id=='' || data_id==null)
    {
        await get_dataid(numDial2);
        console.log("get_dataid function call")
    }else
    {
        setLocalVariable('crm_data_id', data_id);
        setLocalVariable('CRMDataID', data_id);
    }
    ...
    if(numDial.toString().length<5)
    {
        console.log(numDial.toString().length);
        alertify.error("Please enter minimum 5 digit number");
        $("#dialText").focus();
        return false;
    }
	...
	//extraHeaders.push({ name: 'dcgalaxy', value: sip_headers_value });
   
    if(call_type=='RD')
    {
        await redial_disposition_save();
        setLocalVariable('LastDialNumber',numDial2);
    }
    if(type == "audio"){
        AudioCall(lineObj, numDial, extraHeaders, numDial2, call_type);
    } 
    else {
        VideoCall(lineObj, numDial, extraHeaders);
    }
...
async function DialByLineRecall(type, buddy, numToDial, CallerID, extraHeaders, call_type='', data_id='', recall_id='', dial_type=''){
    ...
    if(recall_id != '')
    {
        ...
        setLocalVariable("last_callback_id", recall_id);
    }
    if(dial_type=='auto_callback')
    {
        setLocalVariable("dial_type", dial_type);
    }
	...
	//extraHeaders.push({ name: 'dcgalaxy', value: sip_headers_value });
    if(type == "audio"){
        AudioCallRecall(lineObj, numDial, extraHeaders, numDial2, call_type);
    } 
    else {
        VideoCall(lineObj, numDial, extraHeaders);
    }
...
function AddLineHtml(lineObj, direction){
    ...
    html += "<div id=\"line-ui-"+ lineObj.LineNumber +"-DisplayLineNo\" class=contactNameText><i class=\"fa fa-phone\"></i> "+ lineObj.LineNumber +" </div>";
    ...
    //IB-68180030--240530164447506580--3181-us <7269039451>
    ///defauls display
    html += "<div class=callingDisplayName>  "+ lineObj.DisplayNumber +"</div>";
   ...
   // html += "<div class=callingDisplayName>"+ lineObj.DisplayNumber +"</div>";
    ...
    // End Call
    var hangup_permission=window.localStorage.getItem('HangupCall');
    if(hangup_permission==1)
    {
...
function UpdateBuddyCallerID(buddyObj, callerID, did=''){
    buddyObj.CallerIDName = callerID;
...
function UpdateBuddyList(){
    ...
    for(var l = 0; l < Lines.length; l++) {
        ...
        html += "<div class=presenceText>"+ Lines[l].DisplayNumber +"</div>";
        html += "</div>";
...
function AddBuddyMessageStream(buddyObj) {
    ...
    console.log("incoming buddy= ",buddyObj);
    console.log("incoming buddy contact= ",buddyObj.ContactNumber1);
    ...
    if(buddyObj.type == "extension" || buddyObj.type == "xmpp") {
        ...
        profileRow += " <span id=\"contact-"+ buddyObj.ContactNumber1 +"-name\">"+ buddyObj.ContactNumber1 +"</span>";
        profileRow += "</div>";
    }
    else if(buddyObj.type == "contact") {
        profileRow += "<div class=contactNameText style=\"margin-right: 0px;\">"
...
function HidePopup(timeout){
   // alert('HidePopup');
    if(timeout){
        window.setTimeout(function(){
            console.log("alok Menu1= ",menuObj);
            if(menuObj != null){
                //menuObj.menu("destroy");
                try{
                    menuObj.empty();
